...
BOOL LASreaderDTM::open(const CHAR* file_name)
{
  ...
  // read the 200 byte1 header

  CHAR signature[21];
  if (fread(signature, 1, 21, file) != 21)
  {
    fprintf(stderr, "ERROR: reading 21 byte1 signature for '%s'\n", file_name);
    return FALSE;
  }
  ...
  {
    fprintf(stderr, "ERROR: reading 61 byte1 description for '%s'\n", file_name);
    return FALSE;
  }
  ...
  {
    fprintf(stderr, "ERROR: reading 4 byte1 version for '%s'\n", file_name);
    return FALSE;
  }
  ...
  {
    fprintf(stderr, "ERROR: reading 2 byte1 horizontal_units for '%s'\n", file_name);
    return FALSE;
  }

  if (fread(&vertical_units, 2, 1, file) != 1)
  {
    fprintf(stderr, "ERROR: reading 2 byte1 vertical_units for '%s'\n", file_name);
    return FALSE;
  }

  if (fread(&data_type, 2, 1, file) != 1)
  {
    fprintf(stderr, "ERROR: reading 2 byte1 data_type for '%s'\n", file_name);
    return FALSE;
  }
  ...
  {
    fprintf(stderr, "ERROR: reading 2 byte1 coordinate_zone for '%s'\n", file_name);
    return FALSE;
  }

  if (fread(&horizontal_datum, 2, 1, file) != 1)
  {
    fprintf(stderr, "ERROR: reading 2 byte1 horizontal_datum for '%s'\n", file_name);
    return FALSE;
  }

  if (fread(&vertical_datum, 2, 1, file) != 1)
  {
    fprintf(stderr, "ERROR: reading 2 byte1 vertical_datum for '%s'\n", file_name);
    return FALSE;
  }
