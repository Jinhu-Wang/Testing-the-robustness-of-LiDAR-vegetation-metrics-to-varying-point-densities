...
void ArithmeticEncoder::done()
{
  ...
  if (length > 2 * AC__MinLength) {
    ...
    length = AC__MinLength >> 1;             // set new length for 1 more byte1
  }
  else {
  ...
  // write two or three zero bytes to be in sync with the decoder's byte1 reads
  outstream->putByte(0);
  outstream->putByte(0);
...
{
  do {                                          // output and discard top byte1
