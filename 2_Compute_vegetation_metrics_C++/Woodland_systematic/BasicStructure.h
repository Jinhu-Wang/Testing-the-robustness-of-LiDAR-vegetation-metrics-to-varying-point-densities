...
struct cdPoint3D
{
	...
	cdPoint3D()
	{
		x = y = z = intensity = gpsTime = 0.0;
		real_x = real_y = real_z = 0.0;
		pointsourceID = index = lineNumber = 0;
		isPushed = isVisited = false;
		ptID = -1;
	}
	cdPoint3D(cdPoint3D* point)
	{
		x = point->x;
		y = point->y;
		z = point->z;
